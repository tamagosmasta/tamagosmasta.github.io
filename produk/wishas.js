...
import { comentarService } from "./comentarService.js";
...
export const wishas = () => {
    ...
    const initialComentar = async () => {
        ...
        try {
            ...
            if (comentar.length > 0) {
                peopleComentar.textContent = `${comentar.length} Orang telah mengulas`;
            } else {
                peopleComentar.textContent = `Belum ada yang mengulas`;
            }
    ...
    form.addEventListener("submit", async e => {
        ...
        const comentar = {
            ...
            status: statusMap[e.target.status.value] || "Tidak Diketahui",
